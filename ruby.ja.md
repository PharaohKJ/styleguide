...
- **[SHOULD]** 文字列リテラルには基本的に"..."を使用する。ただし、特殊文字 を解釈させたくない場合のみ、'...'を使用する。 また、原則的にヒアドキュメントは使用しない。
...
- **[MUST]** `if`、`unless`、を `puts "x is true" if x` など後置で用いてはならない。
...
- **[SHOULD]** 明らかに可読性に問題がない場合を除いて、三項演算子はなるべく使用しない。 とくに、括弧が必要なほど条件が複雑な場合や、複数行になってしまう場合は、三項演算子は使用しない。
...
- **[MUST]** ブロック付きメソッド呼び出しでは、`do`/`end` 記法でブロックを書くこと。i.e. blockの副作用が目的
  - **[SHOULD]** ただし、メソッドチェインを行う場合は{ ... }を使用する。
...
- **[MUST]** 複数のクラスの区切りには空行を挿入する。
- **[MUST]** クラス内の各構成要素の区切にも空行を挿入する。ただし、最初の構成要素の前や、最後の構成要素の後には空行は挿入しない。
...
## 命名規則
- 全般
  - **[MUST]** 原則として、単語の省略は行わない。
  - **[MUST]** スコープが狭いループ変数には、`i`, `j`, `k`という名前をこの順序で使用する。
  - **[MUST]** スコープが狭い変数名には、クラス名を省略したものを使用してよい。 (例: eo = ExampleObject.new)
  - **[MUST]** クラス・モジュール名は、各単語の一文字目を大文字にし、`_`などの区切 文字は使用しない。ただし、HTTPなどの略語の場合はすべて大文字のままとする。
  - **[MUST]** メソッド名は、すべて小文字とし、単語の区切りに`_`を用いる。メソッド名には動詞の原形を使用する。
  - **[MUST]** 真偽値を返すメソッド名は、動詞または形容詞に`?`を付け、形容詞に`is_`は付けない。
  - **[MUST]** また、破壊的なメソッドと非破壊的なメソッドの両方を提供する場合、破壊的なメソッドには`!'を付ける。
  - **[MUST]** 変数名は、すべて小文字とし、単語の区切りに`_'を用いる。
  - **[MUST]** ファイル名はファイル名は、すべて小文字とし、単語の区切りに`-'を用いる。 また、ファイル中の主な定義クラスの名前を変換したものをファイル名 に使用する。(モジュールを名前空間として使用する場合は、ディレクトリを使用して階層構造を表現する。)
